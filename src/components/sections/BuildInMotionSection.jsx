...
const supportTiles = [
  {
    label: "Village",
    line: "Peer community, live calls, and phase-based groups.",
    symbol: "◦",
  },
  {
    label: "Momentum",
    line: "Checklists, progress tracking, and guided action steps.",
    symbol: "◦",
  },
  {
    label: "Real Answers",
    line: "Q&A, implementation feedback, and support when questions come up.",
    symbol: "◦",
  },
];
    ...
    <section style={{ backgroundColor: "#F0EBE1", borderTop: "1px solid rgba(196,149,106,0.08)" }}>
      ...
      {visible && (
        <style>{`
          ...
          .bim-strip { animation: bimFadeUp 0.6s ease forwards 0.80s; opacity: 0; }
          .bim-t0    { animation: bimFadeUp 0.5s ease forwards 0.88s; opacity: 0; }
          .bim-t1    { animation: bimFadeUp 0.5s ease forwards 0.96s; opacity: 0; }
          .bim-t2    { animation: bimFadeUp 0.5s ease forwards 1.04s; opacity: 0; }
        `}</style>
      )}
      ...
      {/* ── DESKTOP layout ── */}
      <div ref={ref} className="hidden md:block max-w-6xl mx-auto px-10 lg:px-14 pt-14 pb-12">

        {/* Two-column: image left, text right */}
        <div style={{ display: "flex", gap: "clamp(44px, 5vw, 72px)", alignItems: "center" }}>

          {/* Left: floating device mockup */}
          <div
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "11px", marginBottom: "20px" }}>
              {progressMoments.map((moment, i) => (
                ...
                >
                  <span style={{ flexShrink: 0, width: "5px", height: "5px", borderRadius: "50%", backgroundColor: "#C4956A", marginTop: "6px" }} />
                  <div>
        ...
        {/* ── Support strip — below image, desktop only ── */}
        <div
          className={visible ? "bim-strip" : ""}
          style={{
            marginTop: "36px",
            paddingTop: "28px",
        ...
        >
          ...
          <div style={{ marginBottom: "18px" }}>
            <p className="font-display" style={{ color: "#2C2C2C", fontSize: "1.05rem", lineHeight: "1.2", marginBottom: "4px" }}>
              Support that keeps you moving
            </p>
            <p className="font-body" style={{ color: "#7A6E65", fontSize: "0.82rem", lineHeight: "1.55", maxWidth: "58ch" }}>
              You are not building alone. Mama Launch Studio gives you the structure, support, and small next steps to keep going.
            </p>
          </div>

          {/* Three compact tiles */}
          <div style={{ display: "flex", gap: "12px" }}>
            {supportTiles.map((tile, i) => (
              <div
                key={tile.label}
                className={visible ? `bim-t${i}` : ""}
                style={{
                  flex: 1,
                  opacity: visible ? undefined : 0,
                  padding: "14px 16px",
              ...
              >
                <p className="font-body" style={{ color: "#2C2C2C", fontSize: "0.84rem", fontWeight: 600, marginBottom: "4px" }}>
                  {tile.label}
                </p>
                <p className="font-body" style={{ color: "#7A6E65", fontSize: "0.78rem", lineHeight: "1.5" }}>
                  {tile.line}
                </p>
              </div>
            ))}
